...
public class Hooks{
	...
	@Before("@APIFeature") // for step definitions to recognise
	public void setUpAPI(Scenario scenario) throws JDOMException, IOException
	{
		testcontext.objXMLReader.loadTestDataXML(System.getProperty("user.dir")+"\\src\\test\\resources\\TestData\\XMLData\\practise.xml");
		testcontext.objXMLReader.getTCNameFromChildren("APIToGetPlaceID");

		String FName = FilenameUtils.getBaseName(scenario.getUri().toString());//to feature file name
		if(FName.equalsIgnoreCase("Api"))
		{
			if(TestContext.API_extent==null)
			{
				TestContext.API_extent = new ExtentReports();
				TestContext.API_extentReporter = new ExtentSparkReporter("C:\\Meenusha\\Reports1\\"+FName+".html");
				TestContext.API_extent.attachReporter(TestContext.API_extentReporter);
				TestContext.API_extentReporter.config().setTheme(Theme.DARK);
				TestContext.API_extentReporter.config().setTheme(Theme.DARK);
			}
			testcontext.API_test = TestContext.API_extent.createTest(scenario.getName());
			testcontext.api.test = this.testcontext.API_test;

			testcontext.api.json = this.testcontext.json;
			testcontext.api.resp = this.testcontext.resp;
			//Excel_TotalTC.add(scenario.getName());
		}
	}

	@After("@APIFeature") // for step definitions to recognise
	public void TearDownAPI(Scenario scenario) throws JDOMException, IOException
	{
		testcontext.API_test.pass(scenario.getName() +" - completed Succesfully");
		TestContext.API_extent.flush();
		count(scenario);
	}
}
